...
class GUI(Tk):
    ...
    def observation_length_options(self, first_last_year):
        ...
        if len(options) < 1:
            text_ = str('[Fehler] Leider lässt die Datenlage keine Analyse in diesem Modus zu.')
            self.start_thread(task=self.update_output_scrollbar(text_), task_name='text_update')
            self.widget_state(widget=self.start_button, state=False)
    ...
    def with_selecting_dataset_or_modus(self, event):
        ...
        self.updated_first_last_year = (self.least_valid_year, self.first_last_year[1])
        self.observation_length_options(self.updated_first_last_year)
    ...
    def update_modus_options(self):
        options_init = ['CDD (Cooling-Degree-Day)', 'HDD (Heating-Degree-Day)', 'TMK (Tagesmittel Temperatur)', 'RSK (Tägliche Niederschlagshöhe)',
                        'SDK (Tägliche Sonnenscheindauer)', 'SHK (Tageswert Schneehöhe)', 'NM (Tagesmittel Bedeckungsgrad)', 'VPM (Tagesmittel Dampfdruck)',
                        'PM (Tagesmittel Luftdruck)', 'UPM (Tagesmittel relative Feuchte)', 'TXK (Tagesmaximum Lufttemperatur)', 'TNK (Tagesminimum Lufttemperatur)']
        options_df = self.data_df.columns.values
        updated_options = [option_ for option_ in options_init if option_[:3] in options_df]
    ...
    def core_calculation_and_plot_function(self, level):
        ...
        if level == 0:
            ...
            self.start_thread(task=self.update_calculation_scrollbar(self.raw_data), task_name='text_raw_data')

            # -------------
            # update_calculation_scrollbar/ data_df
            text_ = str('Errechnete Messwerte aus diesem Datensatz:')
            self.start_thread(task=self.update_calculation_scrollbar(text_), task_name='text_update')
            self.start_thread(task=self.update_calculation_scrollbar(self.data_df), task_name='text_data_df')
            ...
            if len(self.compared_date_range) > 1:
                ...
                self.start_thread(task=self.update_calculation_scrollbar(text_), task_name='text_update')
                if len(self.compared_date_range) < 10: self.start_thread(task=self.update_calculation_scrollbar(str(self.compared_date_range)), task_name='text_compared_date_range')
                else: self.start_thread(task=self.update_calculation_scrollbar(str(self.compared_date_range[:6]) + '\n...\n' + str(self.compared_date_range[-6:])), task_name='text_compared_date_range')

            # -------------
            # valid_data_range/ obervation_sum_df
            self.obervation_sum_df = self.calculate_observation_sum(first_last_year_=self.updated_first_last_year, data_df_=self.data_df, mode_=self.current_modus_menu_value)
    ...
    def replace_missing_value_999(self, raw_values_, values_to_replace_, new_value_):
        ...
        cleaned_values = where(cleaned_values == values_to_replace_[1], new_value_, cleaned_values)
    ...
    def create_data_df(self, raw_data_):
        ...
        data_df = data_df.fillna(data_df.rolling(21, min_periods=1).median())
...
def main_func():
    root = GUI()
    root.title('Wetterderivate und Klima v_b0.7.0')
